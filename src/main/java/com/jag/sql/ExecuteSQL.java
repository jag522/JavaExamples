...
public class ExecuteSQL {
	public static void main(String[] args) {
		...
		try {
			...
			String user = "rs";
			String password = "rs";
			String url = "jdbc:oracle:thin:@172.16.45.135:1521:ora9";
			...
			while (true) {
				...
				if (status) { // it was a query and returns a
					// ResultSet
		...
		// Handle exceptions that occur during database
	...
	static void printResultsTable(ResultSet rs, OutputStream output)
		...
		for (int i = 0; i < numcols; i++) { // for each column
			...
			int size = metadata.getColumnDisplaySize(i + 1); //get the column's design size
			if (size == -1)
